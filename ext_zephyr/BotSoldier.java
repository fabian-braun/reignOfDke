...
public class BotSoldier extends Bot {
	...
	private enum MicroStance {
		SAFE, // Retreat a lot, play it safe. Appropriate when harrassing or
				// gathering.
		DEFENSIVE, // Play it somewhat safe, but don't retreat too much because
					// we have something to protect. Appropriate for defending
		AGGRESSIVE // Be somewhat reckless when many allies are nearby.
	...
	private static void turn() throws GameActionException {
		...
		if (Strategy.active == Strategy.SCATTER_HALF
		...
		// if(inHealingState) {
		// Nav.goTo(ourHQ, Nav.Sneak.NO, Nav.Engage.NO,
		// countNumEnemiesAttackingMoveDirs());
		// return;
		// }
		...
		Nav.Engage navEngage = stance == MicroStance.AGGRESSIVE ? Nav.Engage.YES
				: Nav.Engage.NO;

		int destIndex = towerBuildAssignmentIndex;
		if (destIndex == -1)
			destIndex = pastrBuildAssignmentIndex;
		if (destIndex != -1) {
			Nav.goTo(bestPastrLocations[destIndex], Nav.Sneak.NO, navEngage,
					countNumEnemiesAttackingMoveDirs());
			return;
		}

		if (suppressorBuildAssignmentIndex != -1) {
			MapLocation target = MessageBoard.SUPPRESSOR_TARGET_LOCATIONS
					.readFromMapLocationList(suppressorBuildAssignmentIndex);
			Nav.goTo(target, Nav.Sneak.NO, Nav.Engage.NO,
					countNumEnemiesAttackingMoveDirs());
			return;
		}
	...
	private static boolean tryReceivePastrLocations()
			throws GameActionException {
		numPastrLocations = MessageBoard.NUM_PASTR_LOCATIONS.readInt();
		if (numPastrLocations == 0) {
			if (Strategy.active == Strategy.PROXY
					|| Strategy.active == Strategy.PROXY_ATTACK
		...
		for (int i = 0; i < numPastrLocations; i++) {
			bestPastrLocations[i] = MessageBoard.BEST_PASTR_LOCATIONS
					.readFromMapLocationList(i);
		}

		if (Strategy.active == Strategy.ONE_PASTR
				|| Strategy.active == Strategy.ONE_PASTR_SUPPRESSOR
				|| Strategy.active == Strategy.SCATTER
	...
	private static void tryClaimBuildAssignment() throws GameActionException {
		...
		for (int i = 0; i < numPastrLocations; i++) {
			if (MessageBoard.TOWER_BUILDER_ROBOT_IDS
					.checkIfAssignmentUnowned(i)) {
		...
		for (int i = 0; i < numPastrLocations; i++) {
			if (MessageBoard.PASTR_BUILDER_ROBOT_IDS
					.checkIfAssignmentUnowned(i)) {
	...
	private static void tryClaimSuppressorAssignment()
			throws GameActionException {
		int numSuppressors = MessageBoard.NUM_SUPPRESSORS.readInt();
		for (int i = 0; i < numSuppressors; i++) {
			if (MessageBoard.SUPPRESSOR_BUILDER_ROBOT_IDS
					.checkIfAssignmentUnowned(i)) {
	...
	private static void manageBuildAssignment() throws GameActionException {
		if (towerBuildAssignmentIndex != -1) {
			if (!MessageBoard.TOWER_BUILDER_ROBOT_IDS
					.checkIfIOwnAssignment(towerBuildAssignmentIndex)) {
				// check if we can reassign ourselves to the corresponding pastr
				if (MessageBoard.PASTR_BUILDER_ROBOT_IDS
						.checkIfAssignmentUnowned(towerBuildAssignmentIndex)) {
					MessageBoard.PASTR_BUILDER_ROBOT_IDS
							.claimAssignment(towerBuildAssignmentIndex);
			...
			if (!MessageBoard.PASTR_BUILDER_ROBOT_IDS
					.checkIfIOwnAssignment(pastrBuildAssignmentIndex)) {
				// check if we can reassign ourselves to the corresponding tower
				if (MessageBoard.TOWER_BUILDER_ROBOT_IDS
						.checkIfAssignmentUnowned(pastrBuildAssignmentIndex)) {
					MessageBoard.TOWER_BUILDER_ROBOT_IDS
							.claimAssignment(pastrBuildAssignmentIndex);
			...
			if (!MessageBoard.SUPPRESSOR_BUILDER_ROBOT_IDS
					.checkIfIOwnAssignment(suppressorBuildAssignmentIndex))
				suppressorBuildAssignmentIndex = -1;
		}
	}

	private static boolean areaIsSecureEnoughToBuild()
			throws GameActionException {
		int numDefenders = numOtherNonConstructingAlliedSoldiersInRange(here,
				RobotType.SOLDIER.sensorRadiusSquared);
		int numAttackers = visibleEnemies.length;
		if (numAttackers > 0 && numDefenders <= numAttackers + 3)
			return false;
		return true;
	}
	...
	private static boolean tryBuildSomething() throws GameActionException {
		for (int i = 0; i < numPastrLocations; i++) {
			...
			if (here.equals(pastrLoc)) {
				...
				if (!MessageBoard.PASTR_BUILDER_ROBOT_IDS
						.checkIfIOwnAssignment(i)) {
					MessageBoard.PASTR_BUILDER_ROBOT_IDS.claimAssignment(i);
				}
				// if we were going to build a tower, relinquish our claim to
				// that assignment
				if (towerBuildAssignmentIndex != -1) {
					MessageBoard.TOWER_BUILDER_ROBOT_IDS
							.clearAssignment(towerBuildAssignmentIndex);
					towerBuildAssignmentIndex = -1;
				}
				if (MessageBoard.BUILD_PASTRS_FAST.readBoolean()
						|| Util.containsNoiseTower(rc.senseNearbyGameObjects(
								Robot.class, pastrLoc, 2, us), rc)) {
					if (!areaIsSecureEnoughToBuild())
						return false;
			...
			if (here.isAdjacentTo(pastrLoc)) {
				if (!Util
						.containsConstructingRobotOrNoiseTowerExceptAtLocation(
								rc.senseNearbyGameObjects(Robot.class,
										pastrLoc, 2, us), pastrLoc, rc)) {
					if (!areaIsSecureEnoughToBuild())
						return false;

					Direction dirToPastr = here.directionTo(pastrLoc);
					if (buildingHereBlocksAccess(dirToPastr)
							&& rc.canMove(dirToPastr)) {
						// There is an embarrassing failure mode where building
						// the noise tower blocks access to the pastr location,
						// and the pastr builder is not smart enough to route
						// around the noise tower. We can try to ameliorate this
						// by
						// checking whether building here would block access to
						// the pastr. If it would, and if the pastr builder is
						// not
						// in place yet, then we should move to the pastr
						// location and become the pastr builder.
						rc.move(dirToPastr);
					} else {
						...
						// if we were going to build a pastr, relinquish our
						// claim to that assignment
						if (pastrBuildAssignmentIndex != -1) {
							MessageBoard.PASTR_BUILDER_ROBOT_IDS
									.clearAssignment(pastrBuildAssignmentIndex);
							pastrBuildAssignmentIndex = -1;
						}
		...
		// allowed to build a suppressor if we are assigned to or if we have no
		// other assignments and are in harrass mode
		boolean allowedToBuildSuppressor = suppressorBuildAssignmentIndex != -1;
		if (pastrBuildAssignmentIndex == -1
				&& towerBuildAssignmentIndex == -1
				&& MessageBoard.RALLY_GOAL.readRallyGoal() == BotHQ.RallyGoal.HARRASS) {
		...
		if (allowedToBuildSuppressor) {
			int numSuppressors = MessageBoard.NUM_SUPPRESSORS.readInt();
			for (int i = 0; i < numSuppressors; i++) {
				if (!MessageBoard.SUPPRESSOR_JOBS_FINISHED
						.readFromBooleanList(i)) {
					MapLocation suppressorTarget = MessageBoard.SUPPRESSOR_TARGET_LOCATIONS
							.readFromMapLocationList(i);
					int distSq = here.distanceSquaredTo(suppressorTarget);
					if (distSq <= RobotType.NOISETOWER.attackRadiusMaxSquared
							&& distSq >= 10) {
						MessageBoard.SUPPRESSOR_BUILDER_ROBOT_IDS
								.claimAssignment(i);
						MessageBoard.SUPPRESSOR_JOBS_FINISHED
								.writeToBooleanList(i, true);
						if (!areaIsSecureEnoughToBuild())
							return false;
	...
	private static boolean buildingHereBlocksAccess(Direction dir) {
		if (dir.isDiagonal()) {
			boolean voidL1 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateLeft())));
			boolean voidL3 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateLeft().rotateLeft().rotateLeft())));
			boolean voidR1 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateRight())));
			boolean voidR3 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateRight().rotateRight().rotateRight())));
			return (voidL1 && voidR1) || (voidL1 && voidR3)
					|| (voidL3 && voidR1) || (voidL3 && voidR3);
		} else {
			boolean voidL2 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateLeft().rotateLeft())));
			boolean voidR2 = !Util.passable(rc.senseTerrainTile(here.add(dir
					.rotateRight().rotateRight())));
			return (voidL2 && voidR2);
		}
	}

	private static void constructNoiseTower(MapLocation pastrLoc)
			throws GameActionException {
		rc.construct(RobotType.NOISETOWER);
	...
	private static void updateEnemyData() throws GameActionException {
		Robot[] visibleEnemyRobots = rc.senseNearbyGameObjects(Robot.class,
				RobotType.SOLDIER.sensorRadiusSquared, them);
		...
		for (int i = visibleEnemyRobots.length; i-- > 0;) {
			...
			if (visibleEnemies[i].type == RobotType.SOLDIER
					&& !visibleEnemies[i].isConstructing)
				numVisibleNonConstructingEnemySoldiers++;
		}

		numNonConstructingSoldiersAttackingUs = 0;
		if (visibleEnemies.length > 0) {
			Robot[] attackableEnemyRobots = rc
					.senseNearbyGameObjects(Robot.class,
							RobotType.SOLDIER.attackRadiusMaxSquared, them);
	...
	private static void manageHealingState() {
		if (!tryingSelfDestruct) {
			if (health < 20) {
				inHealingState = true;
				// Debug.indicate("heal", 0,
				// "<20 health: going to healing state");
				...
				for (int i = attackableEnemies.length; i-- > 0;) {
					...
					if (enemy.actionDelay < 3
							&& enemy.type == RobotType.SOLDIER)
						numAttackers++;
				}
				if ((numAttackers + 1) * RobotType.SOLDIER.attackPower >= health) {
					// Debug.indicate("heal", 0, "" + numAttackers +
					// " attackers: going to healing state");
					inHealingState = true;
				} else {
					// Debug.indicate("heal", 0,
					// "not going to healing state: health = " + health +
	...
	private static MicroStance chooseMicroStance(MapLocation rallyLoc,
			BotHQ.RallyGoal rallyGoal) {
		switch (rallyGoal) {
		case DEFEND:
			...
			// We are close to the rally point. But if there is still an enemy
			// between us and the rally point, we have to be aggressive
			MapLocation ahead = here.add(here.directionTo(rallyLoc), 2);
			if (rc.senseNearbyGameObjects(Robot.class, ahead,
					RobotType.SOLDIER.attackRadiusMaxSquared, them).length > 0) {
		...
		case HARRASS:
			if (here.distanceSquaredTo(rallyLoc) > 100)
				return MicroStance.AGGRESSIVE; // punch through to the target
			else
				return MicroStance.SAFE; // Don't lose units
	...
	private static boolean doVoluntaryMicro(MapLocation rallyLoc,
			BotHQ.RallyGoal rallyGoal, Nav.Sneak sneak)
			throws GameActionException {
		switch (rallyGoal) {
		case GATHER:
		case DEFEND:
			if (needToClearOutRallyBeforeDefending)
				return false;
			if (MessageBoard.COLLAPSE_TO_PASTR_SIGNAL.readInt() >= Clock
					.getRoundNum() - 1)
				return false;
			if (here.distanceSquaredTo(rallyLoc) <= 36) {
				MapLocation closestEnemy = Util
						.closestNonHQ(visibleEnemies, rc);
				if (closestEnemy == null)
					return false;
				tryMoveTowardLocationWithNoEnemyExposureWithinRadius(
						closestEnemy, rallyLoc, 36, sneak);
				return true;
			}
			return false;
		...
		case HARRASS:
			if (here.distanceSquaredTo(rallyLoc) <= 81) {
				if (tryMoveTowardLocationWithMaxEnemyExposure(rallyLoc, 0,
						sneak)) {
					// Debug.indicate("micro", 0,
					// "voluntary: moving safely toward rally in harrass mode");
				}
				return true;
			}
			return false;
	...
	// obligatory micro is attacks or movements that we have to do because
	// either we or a nearby ally is in combat
	private static boolean doObligatoryMicro() throws GameActionException {
		...
		if (Clock.getRoundNum() >= MessageBoard.SELF_DESTRUCT_LOCKOUT_ROUND
				.readInt()
				|| rc.getRobot().getID() == MessageBoard.SELF_DESTRUCT_LOCKOUT_ID
						.readInt()) {
			if (numNonConstructingSoldiersAttackingUs >= 1
					&& tryMoveForSelfDestruct()) {
				// Debug.indicate("micro", 0,
				// "trying for one-move self-destruct");
				tryingSelfDestruct = true;
				return true;
			} else if (tryMoveForTwoMoveSelfDestruct()) {
				// Debug.indicate("micro", 0,
				// "trying for two-move self-destruct");
				tryingSelfDestruct = true;
		...
		if (numNonConstructingSoldiersAttackingUs >= 1) { // we are in combat
			// If we are getting double-teamed worse than any of the enemies we
			// are fighting, try to retreat
			...
			for (int i = attackableEnemies.length; i-- > 0;) {
				...
				// our soldiers to defend buildings:
			...
			if (numNonConstructingSoldiersAttackingUs == 1) {
				if (maxAlliesAttackingEnemy == 1) {
					// we are in a 1v1. fight if we are winning, otherwise
					// retreat
					RobotInfo singleEnemy = Util
							.findANonConstructingSoldier(attackableEnemies);
					boolean weAreWinning1v1 = health >= singleEnemy.health;
					if (weAreWinning1v1) {
						// Debug.indicate("micro", 0,
						// "attacking in winning 1v1");
						attackAndRecord(singleEnemy);
					} else {
						if (!inHealingState && stance == MicroStance.AGGRESSIVE
								&& guessIfFightIsWinning()) {
							// Debug.indicate("micro", 0,
							// "remaining in losing 1v1 because we are aggressive and fight is winning");
							attackAndRecord(singleEnemy);
						} else {
							// Debug.indicate("micro", 0,
							// "retreating from losing 1v1");
							retreatOrFight();
						}
					}
				} else {
					...
					if (inHealingState && health < singleEnemy.health) {
						// if we are trying to heal, don't stay in the fight if
						// we have less health than the enemy
						retreatOrFight();
					} else {
						// otherwise kill the enemy
						// Debug.indicate("micro", 0,
						// "attacking outnumbered lone enemy");
						attackAndRecord(singleEnemy);
					}
				}
			} else if (inHealingState
					|| stance == MicroStance.SAFE
				...
				// - we are getting double-teamed worse than the enemy and in
				// either a defensive stance or in a losing overall fight
				// Debug.indicate("micro", 0, "outnumbered: retreating");
				retreatOrFight();
			} else {
				// Several enemies can attack us, but we are also double teaming
				// the enemy badly enough to keep fighting
				// Debug.indicate("micro", 0,
				// "we have a good enough double-team not to retreat: attacking");
				attackANonConstructingSoldier();
			}
			return true;
		} else {// we are not in combat.
			// We are only obligated to do something here if a nearby ally is in
			// combat and
			// we can attack a soldier who is attacking them. Then we have to
			// decide whether to move to attack such a soldier
			// and if so which one. Actually, there is one other thing that is
			// obligatory, which is to move to attack
			// helpless enemies (buildings and constructing soldiers) if it is
			// safe to do so.
			if (!inHealingState && stance != MicroStance.SAFE) {
				MapLocation closestEnemySoldier = Util
						.closestNonConstructingSoldier(visibleEnemies, here);
				if (closestEnemySoldier != null) {
					// int numAlliesFighting =
					// numOtherAlliedSoldiersInAttackRange(closestEnemySoldier);
					// we deliberately include buildings in this count to
					// encourage our soldiers to defend buildings:
					...
					if (numAlliesFighting > 0) {
						// Approach this enemy if doing so would expose us to at
						// most numAlliesFighting enemies.
						// int maxEnemyExposure = numAlliesFighting + 1;
						int maxEnemyExposure = stance == MicroStance.AGGRESSIVE ? numAlliesFighting + 1
								: Math.min(numAlliesFighting + 1, 3);
						if (tryMoveTowardLocationWithMaxEnemyExposure(
								closestEnemySoldier, maxEnemyExposure,
								Nav.Sneak.NO)) {
							// Debug.indicate("micro", 0,
							// "moving to engage to assist ally");
							return true;
						}
					}
				}
			...
			// If we didn't have to go help an ally, or were unable to, check if
			// there is something nearby we can shoot.
			// If so, shoot it. It must be a constructing solder or building
			// because it's not a non-constructing soldier
			if (attackableEnemies.length > 0) {
				// Actually, we need to prevent the following situation: we can
				// see a noise tower and a pastr, but are only in
				// range of the noise tower, so we attack the noise tower
				// instead of moving to attack the pastr. It's important
				// to not waste any rounds killing pastrs. So check if we can
				// see a pastr but not shoot a pastr
				if (tryMovetoEngageUndefendedPastr()) {
					// Debug.indicate("micro", 0,
					// "moving to engage undefended pastr");
					return true;
				}
			...
			// We didn't have to help an ally or shoot a helpless enemy. One
			// final possibility with an obligatory response:
			// if we can engage a helpless enemy and not get shot, do it
			if (tryMoveToEngageAnyUndefendedHelplessEnemy()) {
				// Debug.indicate("micro", 0,
				// "moving to engage a helpless enemy");
				return true;
			}

			// If we are trying to heal, don't let anyone with more health than
			// us get too close
			if (inHealingState) {
				...
				for (RobotInfo enemy : visibleEnemies) {
					if (enemy.type == RobotType.SOLDIER
							...
							&& here.distanceSquaredTo(enemy.location) <= 35
							&& enemy.health > health) {
						tooClose = true;
						break;
					}
			...
			// If none of the above cases compelled us to action, there is no
			// obligatory action
			return false;
		}
	}
	...
	private static boolean fleeSelfDestruct() throws GameActionException {
		...
		for (RobotInfo info : attackableEnemies) {
			if (info.type != RobotType.SOLDIER || info.isConstructing)
				continue;
			if (info.location.isAdjacentTo(here)
					&& info.health > RobotType.SOLDIER.attackPower) {
				adjacentEnemySoldier = info;
				break;
			}
		}
		if (adjacentEnemySoldier == null)
			return false;

		// Robot[] endangeredAllies = rc.senseNearbyGameObjects(Robot.class,
		// adjacentEnemySoldier.location, 2, us);
		...
		// }
		//
		// // If he risks more health than us, let him self-destruct, we don't
		// care
		// if (totalEndangeredAlliedHealth <= adjacentEnemySoldier.health)
		// return false;
		...
		Direction[] fleeDirs;
		if (away.isDiagonal()) {
			fleeDirs = new Direction[] { away, away.rotateLeft(),
					away.rotateRight(), away.rotateLeft().rotateLeft(),
					away.rotateRight().rotateRight() };
		} else {
			Direction toBomber = here
					.directionTo(adjacentEnemySoldier.location);
			if (!rc.canMove(toBomber.rotateLeft().rotateLeft())) { // cheap
																	// approximate
																	// test for
																	// whether
																	// there is
																	...
																	// toward
																	// enemy
				fleeDirs = new Direction[] { away.rotateLeft(), away,
						away.rotateRight() };
			} else if (!rc.canMove(toBomber.rotateRight().rotateRight())) { // cheap
																			// approximate
																			// test
																			// for
																			// whether
																			...
																			// toward
																			// enemy
				fleeDirs = new Direction[] { away.rotateRight(), away,
						away.rotateLeft() };
			} else {
				fleeDirs = new Direction[] { away, away.rotateLeft(),
						away.rotateRight() };
		...
		for (Direction fleeDir : fleeDirs) {
			if (!rc.canMove(fleeDir))
				continue;
			if (here.add(fleeDir).isAdjacentTo(adjacentEnemySoldier.location))
				continue;
	...
	// @formatter:off
	// This is a set of two-move paths that we could consider following and then
	// self-destructing
	...
	private static double[] twoMoveSelfDestructPathActionDelay = { 2, 2, 2, 2,
	...
	// MapLocation moveLoc =
	// center.add(twoMoveSelfDestructPaths[i][0]).add(twoMoveSelfDestructPaths[i][1]);
	...
	// }
	// }
	// twoMoveSelfDestructTargetPaths[3+dx][3+dy] is a list of which
	// twoMoveSelfDestructPaths would bring
	// us adjacent to a target at (here.x + dx, here.y + dy)
	private static int[][][] twoMoveSelfDestructTargetPaths = {
			{ { 15 }, { 11, 15 }, { 9, 11, 15 }, { 9, 10, 11 }, { 9, 10, 14 },
					{ 10, 14 }, { 14 } },
			{ { 1, 15 }, { 1, 11 }, { 1, 9, 15 }, { 10, 11 }, { 8, 9, 14 },
					{ 8, 10 }, { 8, 14 } },
			{ { 0, 1, 15 }, { 0, 11, 15 }, { 0, 1, 9, 11, 15 }, { 9, 10, 11 },
					{ 6, 8, 9, 10, 14 }, { 6, 10, 14 }, { 6, 8, 14 } },
			{ { 0, 1, 2 }, { 1, 2 }, { 0, 1, 2 }, {}, { 6, 7, 8 }, { 7, 8 },
					{ 6, 7, 8 } },
			{ { 0, 2, 12 }, { 0, 4, 12 }, { 0, 2, 3, 4, 12 }, { 3, 4, 5 },
					{ 3, 5, 6, 7, 13 }, { 5, 6, 13 }, { 6, 7, 13 } },
			{ { 2, 12 }, { 2, 4 }, { 2, 3, 12 }, { 4, 5 }, { 3, 7, 13 },
					{ 5, 7 }, { 7, 13 } },
			{ { 12 }, { 4, 12 }, { 3, 4, 12 }, { 3, 4, 5 }, { 3, 5, 13 },
					{ 5, 13 }, { 13 } } };
	...
	// }
	// }
	// blockedTwoMoveSelfDestructPath[3+dx][3+dy] is the index of the
	// twoMoveSelfDestructPath that ends on
	// (here.x + dx, here.y + dy), or -1 if there is no such path.
	private static int[][] blockedTwoMoveSelfDestructPath = {
			{ -1, -1, -1, -1, -1, -1, -1 }, { -1, 15, 11, 9, 10, 14, -1 },
			{ -1, 1, -1, -1, -1, 8, -1 }, { -1, 0, -1, -1, -1, 6, -1 },
			{ -1, 2, -1, -1, -1, 7, -1 }, { -1, 12, 4, 3, 5, 13, -1 },
			{ -1, -1, -1, -1, -1, -1, -1 } };
	// @formatter:on

	// @formatter:off
	// selfDestructChargeWillWork[numTurns][actionDelayFloor] tells whether we
	// can successfully charge an
	// enemy with floor(actionDelay) = actionDelayFloor if our charge will take
	// numTurns turns
	...
	// }
	// }
	// selfDestructNotes[3+dx][3+dy] is a list of directions such that one move
	// in that direction brings you adjacent to
	// an enemy at (here.x + dx, here.y + dy)
	private static final int[][][] selfDestructNotes = {
			{ {}, {}, {}, {}, {}, {}, {}, },
			{ {}, { 7 }, { 6, 7 }, { 5, 6, 7 }, { 5, 6 }, { 5 }, {}, },
			{ {}, { 0, 7 }, { 0, 6 }, { 0, 4, 5, 7 }, { 4, 6 }, { 4, 5 }, {}, },
			{ {}, { 0, 1, 7 }, { 1, 2, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 2, 3, 5, 6 }, { 3, 4, 5 }, {}, },
	...
	private static Direction[] sortedOneMoveSelfDestructDirs = {
	...
	private static boolean tryMoveForSelfDestruct() throws GameActionException {
		...
		for (RobotInfo info : attackableEnemies) {
			...
			int[] moves = selfDestructNotes[3 + info.location.x - here.x][3
					+ info.location.y - here.y];
			for (int dir : moves) {
		...
		if (bestDir != null) {
			...
			if (!Bot.isInTheirHQAttackRange(dest)
				...
				double damageTaken = numAttacksSuffered
						* RobotType.SOLDIER.attackPower;
				if (health > damageTaken) {
					double actualMaxDamage = GameConstants.SELF_DESTRUCT_BASE_DAMAGE
							+ GameConstants.SELF_DESTRUCT_DAMAGE_FACTOR
							* (health - damageTaken);
					...
					if (actualTotalDamageDealt > health && numKills > 0) {
						rc.move(bestDir);
						// Tell others not to go for a self-destruct until the
						// round after we self-destruct
						MessageBoard.SELF_DESTRUCT_LOCKOUT_ID.writeInt(rc
								.getRobot().getID());
						MessageBoard.SELF_DESTRUCT_LOCKOUT_ROUND.writeInt(Clock
								.getRoundNum() + 2);
						...
						// Debug.indicate("selfdestruct", 2,
						// "1move: no kill or too little damage");
					}
				} else {
					// Debug.indicate("selfdestruct", 2,
					// "1move: would get shot down");
				}
			} else {
				// Debug.indicate("selfdestruct", 2,
				// "1move: dest in their HQ or next to ally");
			}
		} else {
	...
	private static boolean tryMoveForTwoMoveSelfDestruct()
			throws GameActionException {
		double maxDamage = GameConstants.SELF_DESTRUCT_BASE_DAMAGE
				+ GameConstants.SELF_DESTRUCT_DAMAGE_FACTOR * health;
		int numPaths = twoMoveSelfDestructPaths.length;
		boolean onRoad = rc.senseTerrainTile(here) == TerrainTile.ROAD;
		int chargeTurns = onRoad ? 2 : 3; // how many turns our charge will
											// take. This assumes we have small
											// actiondelay for diagonal charges
		boolean[] actionDelayIsVulnerable = selfDestructChargeWillWork[chargeTurns]; // which
																						// enemy
																						// actiondelays
																						// are
																						// vulnerable
		...
		for (RobotInfo info : visibleEnemies) {
			if (info.type != RobotType.SOLDIER || info.isConstructing)
				continue;
			if (actionDelayIsVulnerable[(int) info.actionDelay]) { // check if
																	// this
																	// enemy
																	// will be
																	// able to
				...
				if (enemyDxP3 >= 0 && enemyDxP3 <= 6 && enemyDyP3 >= 0
						&& enemyDyP3 <= 6) {
					int blockedPath = blockedTwoMoveSelfDestructPath[enemyDxP3][enemyDyP3];
					if (blockedPath != -1)
						pathScores[blockedPath] = -999999;
					double score = Math.min(maxDamage, info.health);
		...
		for (int i = 0; i < numPaths; i++) {
			if (pathScores[i] > bestScore) {
				...
				if (rc.canMove(move1)) {
					...
					if (Util.passable(rc.senseTerrainTile(here.add(move1).add(
							move2)))) {
		...
		if (bestPath != -1) {
			...
			if (rc.senseTerrainTile(here) == TerrainTile.ROAD)
				totalActionDelay *= GameConstants.ROAD_ACTION_DELAY_FACTOR;
			totalActionDelay += rc.getActionDelay();

			// If waiting a turn to recover action delay will not slow us down,
			// then wait: it just lets us avoid
			// taking unnecessary hits
			if (totalActionDelay - (int) totalActionDelay < rc.getActionDelay())
				return true;
			...
			for (RobotInfo enemy : visibleEnemies) {
				...
				if (enemy.location.distanceSquaredTo(dest) <= RobotType.SOLDIER.attackRadiusMaxSquared) {
					...
					numAttacksSuffered += (2 + numVulnerableTurns - (enemy.actionDelay < 1 ? 1
							: (int) enemy.actionDelay)) / 2;
					if (enemy.location.isAdjacentTo(dest)) {
						targetHealths[numTargets++] = enemy.health;
					}
				}
			...
			// String.format("considering self-destruct: numVulnerableTurrns = %d, numAttacksSuffered = %d,",
			// numVulnerableTurns, numAttacksSuffered));
			// Debug.indicate("selfdestruct", 1,
			// String.format("health = %f, damageTaken = %f", health,
			// damageTaken));
			if (health > damageTaken) {
				double actualMaxDamage = GameConstants.SELF_DESTRUCT_BASE_DAMAGE
						+ GameConstants.SELF_DESTRUCT_DAMAGE_FACTOR
				...
				if (actualTotalDamageDealt >= 1.3 * health && numKills > 0) {
					...
					// Debug.indicate("selfdestruct", 2,
					// String.format("actualTotalDamageDealt = %f, numKills = %d: going for it",
					// actualTotalDamageDealt, numKills));
					rc.move(move1);
					...
					// Debug.indicate("selfdestruct", 2,
					// String.format("actualTotalDamageDealt = %f, numKills = %d: too little",
					// actualTotalDamageDealt,
					// numKills));
				}
			}
		}
	...
	private static boolean tryMovetoEngageUndefendedPastr()
			throws GameActionException {
		...
		for (int i = visibleEnemies.length; i-- > 0;) {
			if (visibleEnemies[i].type == RobotType.PASTR) {
				...
				if (here.distanceSquaredTo(pastrLoc) <= RobotType.SOLDIER.attackRadiusMaxSquared)
					return false; // we can shoot a pastr, we don't need to move
			}
		}
		if (!canSeePastr)
			return false;
		...
		while (pathLoc.distanceSquaredTo(pastrLoc) > RobotType.SOLDIER.attackRadiusMaxSquared) {
			...
			if (rc.senseTerrainTile(pathLoc) == TerrainTile.VOID)
				return false;
			if (isInTheirHQAttackRange(pathLoc))
				return false;
			Robot[] enemyRobotsAlongPath = rc.senseNearbyGameObjects(
					Robot.class, pathLoc,
					RobotType.SOLDIER.attackRadiusMaxSquared, them);
			if (Util.containsNonConstructingSoldier(enemyRobotsAlongPath, rc))
				return false;
		}
	...
	private static boolean tryMoveToEngageAnyUndefendedHelplessEnemy()
			throws GameActionException {
		visibleEnemyLoop: for (int i = visibleEnemies.length; i-- > 0;) {
			...
			if (Util.isHelpless(enemy)) {
				...
				// Debug.indicate("micro", 1, "see helpless enemy at " +
				// enemyLocation.toString());
				Direction toEnemyDir = here.directionTo(enemyLocation);
				if (!rc.canMove(toEnemyDir))
					continue visibleEnemyLoop;
				MapLocation pathLoc = here;
				while (pathLoc.distanceSquaredTo(enemyLocation) > RobotType.SOLDIER.attackRadiusMaxSquared) {
					...
					Robot[] enemyRobotsAlongPath = rc.senseNearbyGameObjects(
							Robot.class, pathLoc,
							RobotType.SOLDIER.attackRadiusMaxSquared, them);
					if (Util.containsNonConstructingSoldier(
							enemyRobotsAlongPath, rc))
						continue visibleEnemyLoop;
				}

				// the direct path to attack this helpless enemy is free of
				// enemy soldiers. start moving along it
				rc.move(toEnemyDir);
				return true;
			}
		}
	...
	private static boolean tryMoveTowardLocationWithMaxEnemyExposure(
			MapLocation dest, int maxEnemyExposure, Nav.Sneak sneak)
			throws GameActionException {
		int[] numEnemiesAttackingDirs = countNumEnemiesAttackingMoveDirs();

		Direction toEnemy = here.directionTo(dest);
		Direction[] tryDirs = new Direction[] { toEnemy, toEnemy.rotateLeft(),
				toEnemy.rotateRight() };
		for (int i = 0; i < tryDirs.length; i++) {
			Direction tryDir = tryDirs[i];
			if (!rc.canMove(tryDir))
				continue;
			if (numEnemiesAttackingDirs[tryDir.ordinal()] > maxEnemyExposure)
				continue;
			if (isInTheirHQAttackRange(here.add(tryDir)))
				continue;
			// Debug.indicate("micro", 1,
			// String.format("moving toward %s with max enemy exposure %d (actual exposure %d)",
			// dest.toString(), maxEnemyExposure,
			// numEnemiesAttackingDirs[tryDir.ordinal()]));
			if (sneak == Nav.Sneak.NO)
				rc.move(tryDir);
			else
				rc.sneak(tryDir);
			return true;
		...
		// Debug.indicate("micro", 1,
		// String.format("can't move toward %s with max enemy exposure %d",
		// dest.toString(), maxEnemyExposure));
		return false;
	}
	...
	private static boolean tryMoveTowardLocationWithNoEnemyExposureWithinRadius(
			MapLocation dest, MapLocation center, int maxRadiusSq,
		...
		for (int i = 0; i < tryDirs.length; i++) {
			...
			if (existsEnemyAttackingLocation(tryLoc))
				continue;
			if (center.distanceSquaredTo(here.add(tryDir)) > maxRadiusSq)
				continue;
			if (isInTheirHQAttackRange(here.add(tryDir)))
				continue;
	...
	private static int numOtherAlliedSoldiersAndBuildingsInAttackRange(
			MapLocation loc) throws GameActionException {
		return rc.senseNearbyGameObjects(Robot.class, loc,
				RobotType.SOLDIER.attackRadiusMaxSquared, us).length;
	}
	...
	private static int numOtherNonConstructingAlliedSoldiersInAttackRange(
			MapLocation loc) throws GameActionException {
		return numOtherNonConstructingAlliedSoldiersInRange(loc,
				RobotType.SOLDIER.attackRadiusMaxSquared);
	}

	private static int numOtherNonConstructingAlliedSoldiersInRange(
			MapLocation loc, int rangeSq) throws GameActionException {
		int numAlliedSoldiers = 0;
		Robot[] allies = rc.senseNearbyGameObjects(Robot.class, loc, rangeSq,
				us);
	...
	private static void attackANonConstructingSoldier()
			throws GameActionException {
		RobotInfo target = chooseNonConstructingSoldierAttackTarget(attackableEnemies);
		attackAndRecord(target);
	}

	private static RobotInfo chooseNonConstructingSoldierAttackTarget(
		...
		// "chooseNonConstructingSoldierAttackTarget: target = " +
		// ret.location.toString() + ", bestTurnsToKill = " +
		// bestTurnsToKill);
		return ret;
	}
	...
	// If we can only attack buildings or constructing soldiers, this function
	// decides which one to attack.
	// It assumes that the enemies list does not contain any non-constructing
	// soldiers or HQs!
	...
	private static void retreatOrFight() throws GameActionException {
		// If all our opponents have really high action delay, we can fire a
		// last shot
		// and still be able to move before they can return fire. This would
		// most probably
		// happen if an enemy engaged us after several diagonal moves. This
		// could turn
		...
		if (!inHealingState || attackableEnemies.length == 1) {
			...
			if (canOneHitEnemy || !enemyCanShootAtUs) {
				// Debug.indicate("micro", 2, "parthian shot (canOneHitEnemy = "
				// + canOneHitEnemy + ", enemyCanShootAtUs = " +
				// enemyCanShootAtUs + ")");
				attackANonConstructingSoldier();
				return;
			}
	...
	// @formatter:off
	// attackNotes[5+dx][5+dy] is a list of integers dir such that a soldier at
	// here + (dx, dy) can attack the square here.add(Direction.values()[dir])
	...
	private static int[][][] attackNotes = {
			...
			{ {}, {}, {}, { 7 }, { 6, 7 }, { 5, 6, 7 }, { 5, 6 }, { 5 }, {},
					{}, {} },
			{ {}, {}, { 7 }, { 0, 6, 7 }, { 0, 5, 6, 7 }, { 0, 4, 5, 6, 7 },
					{ 4, 5, 6, 7 }, { 4, 5, 6 }, { 5 }, {}, {} },
			{ {}, { 7 }, { 0, 6, 7 }, { 0, 1, 5, 6, 7 },
					{ 0, 1, 2, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 2, 3, 4, 5, 6, 7 }, { 3, 4, 5, 6, 7 }, { 4, 5, 6 },
					{ 5 }, {} },
			{ {}, { 0, 7 }, { 0, 1, 6, 7 }, { 0, 1, 2, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 2, 3, 4, 5, 6, 7 },
					{ 3, 4, 5, 6 }, { 4, 5 }, {} },
			{ {}, { 0, 1, 7 }, { 0, 1, 2, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 2, 3, 4, 5, 6 }, { 3, 4, 5 }, {} },
			{ {}, { 0, 1 }, { 0, 1, 2, 7 }, { 0, 1, 2, 3, 4, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6 },
					{ 2, 3, 4, 5 }, { 3, 4 }, {} },
			{ {}, { 1 }, { 0, 1, 2 }, { 0, 1, 2, 3, 7 },
					{ 0, 1, 2, 3, 4, 6, 7 }, { 0, 1, 2, 3, 4, 5, 6, 7 },
					{ 0, 1, 2, 3, 4, 5, 6 }, { 1, 2, 3, 4, 5 }, { 2, 3, 4 },
					{ 3 }, {} },
	...
	private static Direction chooseRetreatDirection()
			throws GameActionException {
		int repelX = 0;
		int repelY = 0;
		for (int i = visibleEnemies.length; i-- > 0;) {
		...
		if (absRepelX >= 1.5 * absRepelY) {
			...
			retreatDir = repelY > 0 ? Direction.SOUTH_EAST
					: Direction.NORTH_EAST;
		} else {
			retreatDir = repelY > 0 ? Direction.SOUTH_WEST
					: Direction.NORTH_WEST;
		}

		int bestMinEnemyDistSq = 999999;
		for (int j = visibleEnemies.length; j-- > 0;) {
			int enemyDistSq = here
					.distanceSquaredTo(visibleEnemies[j].location);
		...
		for (int i = 0; i < tryDirs.length; i++) {
			Direction tryDir = Direction.values()[(retreatDir.ordinal()
					+ tryDirs[i] + 8) % 8];
			if (!rc.canMove(tryDir))
				continue;
			MapLocation tryLoc = here.add(tryDir);
			if (isInTheirHQAttackRange(tryLoc))
				continue;

			int minEnemyDistSq = 999999;
			for (int j = visibleEnemies.length; j-- > 0;) {
				int enemyDistSq = tryLoc
						.distanceSquaredTo(visibleEnemies[j].location);
	...
	// If one of our pastrs is within our attack range and is about to die, kill
	// it so that the
	...
	private static boolean mercyKillPastrs() throws GameActionException {
		...
		for (int i = ourPastrs.length; i-- > 0;) {
			...
			if (here.distanceSquaredTo(ourPastrs[i]) <= RobotType.SOLDIER.attackRadiusMaxSquared) {
				if (rc.senseRobotInfo(rc.senseNearbyGameObjects(Robot.class,
						pastrLoc, 0, us)[0]).health <= RobotType.SOLDIER.attackPower) {
	...
	private static boolean weAreNearEnemyPastr() {
		...
		for (int i = enemyPastrs.length; i-- > 0;) {
			...
			if (distSq < smallestEnemyDistSq)
				smallestEnemyDistSq = distSq;
		}
		if (smallestEnemyDistSq > 70)
			return false;
		int smallestAllyDistSq = 999999;
		for (int i = numPastrLocations; i-- > 0;) {
			int distSq = here.distanceSquaredTo(bestPastrLocations[i]);
			if (distSq < smallestAllyDistSq)
				smallestAllyDistSq = distSq;
		}
		return smallestEnemyDistSq < smallestAllyDistSq;
	}

	private static void attackAndRecord(RobotInfo enemyInfo)
			throws GameActionException {
		if (enemyInfo == null)
			return; // should never happen, but just to be sure
		rc.attackSquare(enemyInfo.location);
		// if (enemyInfo.health <= RobotType.SOLDIER.attackPower)
		// MessageBoard.ROUND_KILL_COUNT.incrementInt();
	}
