...
public class HQ extends AbstractRobotType {
	...
	if (rc.senseRobotCount() < 1) {
		...
		if (rc.senseRobotCount() > pastrThreshold) {
		    ...
		    if (size(ownPASTRS) < MAXIMUM_OWN_PASTRS) {
			...
			if (size(bestLocations) == 0) {
			    ...
			    System.out.println("End in " + Clock.getRoundNum());
			}
			// MapLocation bestPastrLocation = bestLocations[0];
			// if (Soldier.size(ownPASTRS) == 0) {
			// teams[1].setTask(Task.BUILD_PASTR,
			// bestPastrLocation);
			// } else {
			// for (int i = 0; i < bestLocations.length; i++) {
			// bestPastrLocation = bestLocations[i];
